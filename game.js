...
function stopAllSounds() {
  ...
  engineStarted = false;
  currentEngine = engineSoundA;
  nextEngine = engineSoundB;
}
...
controlsUI.innerText = "Управление: WASD";

controlsUI.style.color = "white";
controlsUI.style.fontFamily = "monospace";
controlsUI.style.fontSize = "18px";
controlsUI.style.marginLeft = "10px";
controlsUI.style.opacity = "0.8";
controlsUI.style.display = "flex";
controlsUI.style.alignItems = "center";
const coordsUI = createGamePanel("X: 0  Y: 0");
coordsUI.style.position = "fixed";
coordsUI.style.top = "20px";
coordsUI.style.right = "20px";

document.body.appendChild(coordsUI);


leftColumn.appendChild(lapsUI);
leftColumn.appendChild(speedUI);
leftColumn.appendChild(lapTimeUI);

uiContainer.appendChild(leftColumn);
uiContainer.appendChild(controlsUI);
...
function createOffsetPath(basePath, offset) {
...
const bots = [
  createBot(2899.4, 860.2, "#0044ff", trackPath),
  createBot(2807.7, 851.8, "#00cc00", greenBotPath),
  createBot(2754.3, 883.2, "#ffcc00", yellowBotPath)
];

const botStartPositions = bots.map(bot => ({
  x: bot.x,
  y: bot.y,
  angle: bot.angle
}));
...
function isGrass(x, y) {
  ...
  if (px < 0 || py < 0 || px >= trackCanvas.width || py >= trackCanvas.height) {
    return true;
  }

  const pixel = trackCtx.getImageData(px, py, 1, 1).data;
  ...
  return (g > r + 20 && g > b + 20 && brightness < 200);
}
...
function updateBot(bot) {
  ...
  // если бот на траве — корректируем курс обратно
  if (isGrass(bot.x, bot.y)) {

    bot.speed *= 0.96;

    const target = bot.path[bot.currentTarget];
    const recoveryAngle = Math.atan2(target.y - bot.y, target.x - bot.x);

    let diff = recoveryAngle - bot.angle;
...
function resetEngineSound() {
  engineSoundA.pause();
  engineSoundB.pause();

  engineSoundA.currentTime = 0;
  engineSoundB.currentTime = 0;
...
restartBtn.addEventListener("click", () => {
  ...
  for (let i = 0; i < bots.length; i++) {
    const bot = bots[i];
    const start = botStartPositions[i];
    bot.x = start.x;
    bot.y = start.y;
    bot.angle = start.angle;
    bot.prevX = bot.x;
    bot.prevY = bot.y;
    bot.speed = 0;
    bot.turnVelocity = 0;
...
if (sectorMessageTimer > 0) {
  ...
  if (
    ...
    px >= 0 &&
    py >= 0 &&
    px < trackCanvas.width &&
    py < trackCanvas.height
  ) {
    const pixel = trackCtx.getImageData(px, py, 1, 1).data;
    ...
    // зелёный доминирует И пиксель не слишком светлый
    if (g > r + 20 && g > b + 20 && brightness < 200) {
      offTrack = true;
    }
  }
  ...
  if (offTrack) {
    ...
    const grassMaxSpeed = 1.5;

    // мягкое ограничение максималки
    if (player.speed > grassMaxSpeed) {
      player.speed -= 0.07;
    }

    if (player.speed < -grassMaxSpeed * 0.5) {
      player.speed += 0.07;
    }

    // лёгкая потеря управляемости
    player.turnVelocity *= 0.97;
  }
...
for (let i = 0; i < sectorLines.length; i++) {
  ...
  if (crossedSector && !sectorPassed[i]) {
    ...
    sectorSound.play();
    showSectorMessage("LAP TIME: " + formatLapTime(lapTimer));
  }
  ...
  if (crossed && !raceFinished) {
    ...
    showSectorMessage("LAP " + lapCount + "  " + formatLapTime(completedLapTime));
  ...
  lapTimeUI.innerText = "TIME: " + formatLapTime(lapTimer);
  coordsUI.innerText =
